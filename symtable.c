...
BTYPE arrayNameCheck(struct SymTable* table, const char* name, vector< BTYPE > arrayCheck){	
	...
	do{
		...
		while(entry!=NULL)
		{
			if(!strcmp(entry->name, name)&& entry->type->isArray){			
				check = true;
				...
				if(arrayCheck.size() == entry->type->dim) return entry->type->baseType;
					...
					printf("##########Error at Line %d: %s array dim wrong ##########\n",linenum, name);
					return TBD;	
				}else return ARR;
				break;
			}else if(strcmp(entry->name, name) == 0 && !entry->type->isArray){
				printf("##########Error at Line %d: %s is not array ##########\n",linenum, name);
				return TBD;	
			}
			entry = entry->next;
		}
...
void arrayDeclareCheck(vector<BTYPE> v1, struct ArrayDimNode* dimNode, BTYPE t1)
{
	...
	if(v1.size() > count)
	{
		printf("##########Error at Line %d: exceed array size ##########\n", linenum);
	}
	for(int i=0;i<(int)v1.size();i++)
	{
		...
		if(c){
			printf("##########Error at Line %d: array initial value type error ##########\n", linenum);
...
void returnTypeCheck(BTYPE t1, BTYPE returnType[100], int return_linenum[100], int return_count)
{
	...
	if(return_count == 0 && t1 != VOID_t){
		printf("##########Error at Line %d: need return value ##########\n", linenum);
	}
...
void functDeclDefParamCheck(struct FuncAttrNode *attr1, struct FuncAttrNode *attr2)
{
	...
	if(attr1 != NULL && attr2 != NULL)
	{
		do
		{
			...
			if(attr1->value->isArray && attr2->value->isArray)
			{
				...
				if(dim1 != NULL || dim2 != NULL) check = 0;
		...
		if(attr1 != NULL || attr2 != NULL) check = 0;
	...
	if(!check)
	{
		printf("##########Error at Line #%d: decl and def param not match.##########\n", linenum);	
	}
}
...
BTYPE functParamInvokeCheck(struct SymTable* table, const char* name, vector<BTYPE> v)
{
			...
			if(strcmp(entry->name, name) == 0 && entry->kind == FUNCTION_t && entry->attr == NULL){check = 1; break;}
				...
				else
				{
					...
					for(i=0;i<(int)(v.size());i++){
						...
						else if(v[i] == ARR && attrNode-> value->isArray){}
						else{check = 4; break;}
					}
	...
	if(check == 0){
		printf("##########Error at Line #%d: %s func no declear.##########\n", linenum,name);		
	}
	else if(check == 1)
	{
		printf("##########Error at Line #%d: %s func doens't need param .##########\n", linenum,name);
	}
	else if(check ==2)
	{
		printf("##########Error at Line #%d: %s is not function.##########\n", linenum,name);
	}
	else if(check == 3)
	{
		printf("##########Error at Line #%d: %s func param num error.##########\n", linenum,name);
	}
	else if(check == 4)
	{
		printf("##########Error at Line #%d: %s func param type error.##########\n", linenum,name);
	}
	return TBD;
...
BTYPE functNoParamInvokeCheck(struct SymTable* table, const char* name)
{
	...
	{
		printf("##########Error at Line #%d: %s needs parameters.##########\n", linenum,name);
	}
	else if(check == 2)
	{
		printf("##########Error at Line #%d: %s not a function.##########\n", linenum,name);
	}	
	return TBD;
}
